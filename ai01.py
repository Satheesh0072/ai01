...
# Safe defaults for date_input
min_ts = df["Date"].min()
max_ts = df["Date"].max()
min_date = min_ts.date() if not pd.isna(min_ts) else datetime.today().date()
max_date = max_ts.date() if not pd.isna(max_ts) else datetime.today().date()

start_pie, end_pie = st.date_input(
